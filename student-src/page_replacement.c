...
pfn_t free_frame(void) {
    ...
    if (frame_table[victim_pfn].mapped) {
        pte_t* table = (pte_t*) (mem+frame_table[victim_pfn].process->saved_ptbr * PAGE_SIZE);
        pte_t* entry = &table[frame_table[victim_pfn].vpn];
        
        if (entry->dirty) {
            uint8_t* fAddr = mem + victim_pfn * PAGE_SIZE;
            swap_write(entry, fAddr);
            entry->dirty = 0;
            stats.writebacks++;
        }
