...
void system_init(void) {
    ...
    memset(frame_table, 0, NUM_FRAMES * PAGE_SIZE);
...
void proc_init(pcb_t *proc) {
    ...
    uint8_t* newFrame = mem + new_frame * PAGE_SIZE;
...
uint8_t mem_access(vaddr_t address, char rw, uint8_t data) {
    ...
    /*
        ...
        table entry.
    */
   uint8_t* pAddress = (uint8_t*) mem + offset + (entry->pfn * PAGE_SIZE);
...
void proc_cleanup(pcb_t *proc) {
    ...
    for (size_t i = 0; i < NUM_PAGES; i++) {
        if (table[i].valid) {
            frame_table[table[i].pfn].mapped = 0;
        }
